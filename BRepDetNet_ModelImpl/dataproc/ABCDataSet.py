...
class ABCDataSet(Dataset):
    def __init__(self, root_path: str, scan_path: str, annotationBndry_path: str, annotationJnc_path: str, subset: str = 'test', transform=None, selectedSamples=None, seed=42, sequence=None, add_pose=False, **kwargs):
        """args:
        ...
        if len(sampleIDs) == 0:
            ...
            for s in tqdm(sampleIDs): #sampleIDs
                corrScanFile = os.path.split(os.path.join(self._scanPath, s.replace('_step_', '_obj_')))[0] + "/" + os.path.splitext(os.path.basename(s).replace('_step_', '_trimesh_'))[0] + ".obj"
                corrBndryAnnotFile = os.path.splitext(os.path.join(self._annotBndryPath, s))[0] + ".npz"
                corrJncAnnotFile = os.path.splitext(os.path.join(self._annotJncPath, s))[0] + ".npz"
    ...
    def __getitem__(self, item):
        t = self.samples[item]  #NOTE <-- Item is the index of the sample from ABCDataSet
        scan = o3d.io.read_triangle_mesh(t['scan'])#, enable_post_processing=True) <-- No need of post-processing, data corrected
