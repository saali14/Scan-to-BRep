...
class FaceIndexValidator:
    ...
    def check_part(self, part, face_map, face_boxes):
        ...
        for face_idx, face in enumerate(faces):
            ...
            if not face_index_ok:
                ...
                box_check_ok = self.check_box(
                    fusion_face_box,
                    bscaled,
                    diag / 10,
                    "Error exceeds 1/10 of face box",
                )
                if not box_check_ok:
    ...
    def check_box(self, fusion_face_box, step_face_box, tol, msg):
        min_in_tol = (
            fusion_face_box.CornerMin().Distance(step_face_box.CornerMin()) < tol
        )
        max_in_tol = (
            fusion_face_box.CornerMax().Distance(step_face_box.CornerMax()) < tol
        )
