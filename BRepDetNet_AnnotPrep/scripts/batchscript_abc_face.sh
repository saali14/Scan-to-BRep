...
ABC_ANNOT_FACE_DIR="$ABC_ROOT_DIR/abc_v1.0_BRepFaceLabels"
...
do
    ...
    python3 -m BRep2CADLabler labelBRepFace2ScanABC --step_path "$ABC_STEP_DIR/$s" --scan_path "$ABC_SCAN_DIR/$m" --output "$ABC_ANNOT_FACE_DIR/$s" --nProc 60 --infmt .step --ofmt .npz >> "$LOGFILE" 2>&1
done
