...
CC3D_ANNOT_FACE_DIR="$CC3D_ROOT_DIR/cc3d_v1.0_BRepFaceLabels"
LOGDIR="$CC3D_ROOT_DIR/logs"
LOGFILE="$LOGDIR/logs_facePrep.txt"
...
for prefix in "${prefixes[@]}"; do
  for i in "${index_array[@]}"; do
    ...
    python3 -m BRep2CADLabler labelBRepFace2Scan --step_path "$CC3D_STEP_DIR/$s" --scan_path "$CC3D_SCAN_DIR/$m" --output "$CC3D_ANNOT_FACE_DIR/$s" --nProc 60 --infmt .step --ofmt .npz --scale_brep=True >> "$LOGFILE" 2>&1
  done
done
